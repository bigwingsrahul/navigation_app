...
import 'package:navigation_app/RouteStateModel.dart';
...
class AppLogic {
  ...
  final ValueChanged<RouteStateModel> _updateRouteState;
  ...
  AppLogic(HereMapController hereMapController, ValueChanged<String> updateMessageState,  ValueChanged<RouteStateModel> updateRouteState, Function showDialogCallback)
      : _hereMapController = hereMapController,
        _updateMessageState = updateMessageState,
        _updateRouteState = updateRouteState,
        _showDialogCallback = showDialogCallback,
        _navigationExample = NavigationExample(hereMapController, updateMessageState, updateRouteState),
  ...
  Future<void> _calculateRoute(bool isSimulated) async {
    ...
    _routeCalculator.calculateTruckRoute(_startWaypoint!, _destinationWaypoint!,
  ...
  Future<void> reRoute(BuildContext context, RouteStateModel data) async {

    _routeCalculator.reRouteTruck(data.route, data.startPoint, data.lastTraveledSectionIndex, data.traveledDistanceOnLastSectionInMeters,
        (routeError, route) {
          if(routeError == null){
            ...
            _navigationExample.reRouteTruck(route!.first);
          }else{
            ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(routeError.toString())));
          }
        });
  }
